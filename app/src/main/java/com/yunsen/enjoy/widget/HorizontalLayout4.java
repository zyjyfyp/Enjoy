...
public class HorizontalLayout4 extends LinearLayout {
    private Context mContext;
    private LayoutInflater mInflater;
    private List<ImgAndTextModel> mData;
    private onHorizontalItemClick mListener;

    public HorizontalLayout4(Context context) {
        super(context);
        initView(context);
    }


    public HorizontalLayout4(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }

    public HorizontalLayout4(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public void upData(List<ImgAndTextModel> data) {
        if (data != null && mData != null && data.size() == mData.size()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                ImageView bottomImg = view.findViewById(R.id.bottom_img);
                TextView topTv = (TextView) view.findViewById(R.id.top_tv);
                bottomImg.setImageResource(mData.get(i).getResId());
                topTv.setText(mData.get(i).getText());
            }
        }
    }
    ...
    private LinearLayout createChild(ImgAndTextModel data) {
        ...
        View view = mInflater.inflate(R.layout.text_img_layout, layout, false);
        TextView topTv = (TextView) view.findViewById(R.id.top_tv);
        ImageView bottomImg = view.findViewById(R.id.bottom_img);
        bottomImg.setImageResource(fData.getResId());
        topTv.setText(fData.getText());
        layout.addView(view);
